...
Agent(description="Run code review",
      ...
      prompt="Review the fix on this branch against the root cause diagnosis.
        ...
        If the fix relies on any framework hook, callback, validator, middleware,
        ...
        3. TEST COVERAGE — Verify tests exist for each distinct input path, not
           just the one that triggered the original bug.
...
Return to the state machine SKILL.md for Gate 5.
